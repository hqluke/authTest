...
const postInsertData = async (req, res) => {
    ...
    console.log("Reps Array:", repsArray);
    console.log("Weight Array:", weightArray);

    // Validation
    const numSets = parseInt(sets);
    if (isNaN(numSets) || numSets <= 0) {
        console.error("Invalid sets value:", sets);
        return res
            .status(400)
            .send("Invalid number of sets <a href='/exercise'>Go Back</a>");
    }

    if (repsArray.length !== numSets || weightArray.length !== numSets) {
        console.error("Mismatch:", {
            expectedSets: numSets,
            repsCount: repsArray.length,
            weightsCount: weightArray.length,
        });
        return res
            .status(400)
            .send(
                "Mismatch between sets and data <a href='/exercise'>Go Back</a>",
            );
    }
    ...
    try {
        ...
        for (let i = 0; i < numSets; i++) {
            ...
            if (weightArray[i] === undefined || repsArray[i] === undefined) {
                console.error(`Missing data for set ${i + 1}`);
                throw new Error(`Missing weight or reps for set ${i + 1}`);
            }

            let weightId = await db.getWeightIdFromWeight(weightArray[i]);

            if (!weightId || !weightId.id) {
                console.error(
                    "Weight ID not found for weight:",
                    weightArray[i],
                );
                throw new Error(`Invalid weight: ${weightArray[i]}`);
            }
        ...
        return res
            .status(500)
            .send(
                `Internal server error: ${error.message} <a href='/exercise'>Go Back</a>`,
            );
    }
};
    ...
    try {
        ...
        if (isNaN(dist) || dist <= 0 || dist > 999.99) {
            return res
                .status(400)
                .send(
                    "Invalid distance. Must be between 0.01 and 999.99 miles",
                );
        }
